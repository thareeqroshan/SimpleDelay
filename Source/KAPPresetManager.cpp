...
{
    const int numParamters = mProcessor->getNumParameters();
...
{
    juce::File presetFile = juce::File(mPresetDirectory + directorySeperator + inPresetName +  PRESET_FILE_EXTENSTION);
...
void KAPPresetManager::storeLocalPreset()
{
    ...
    for (juce::DirectoryIterator di(juce::File(mPresetDirectory),
        false,
        "*" + (juce::String) PRESET_FILE_EXTENSTION,
